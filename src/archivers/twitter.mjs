...
import { tweets } from "../db/schema.mjs";
...
export async function archiveTwitter(start, end) {
  ...
  for (let i = resumeIdx; i < intervals.length; i++) {
    ...
    const { archived, skipped } = await archiveTweetBatch(items);
  ...
  logger.success(`Twitter done: ${totalArchived} archived, ${totalSkipped} skipped`);
}
